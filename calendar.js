...
function handleGetHolidays(evt) {
  ...
  let leapYear =
    (userYear.value % 4 === 0 && userYear.value % 100 !== 0) ||
    userYear.value % 400 === 0;
  ...
  if (userCountry.value === "Please Choose a Country") {
    ...
    alert(
      "February ends on the 28th unless it is a leap year. Please choose a valid date."
    );
  } else if (leapYear && userMonth.value === "2" && userDay.value > 29) {
    alert(
      `The year ${userYear.value} is a Leap Year! February only gets one extra day though. Please choose a valid date.`
    );
  } else if (
    userDay.value === "31" &&
    (userMonth.value === "4" ||
    ...
    alert(
      "The month in question does not have 31 days. Please choose a valid date."
    );
  } else {
    $.ajax({
      ...
      (data) => {
        ...
        if (holidayList === undefined) {
          ...
          alert(
            `There is no data for ${countryName} during ${userYear.value}.  Please input another year.`
          );
        } else {
          renderHoliday();
        }
...
function renderHoliday() {
  if (holidayList.length > 0) {
    ...
    $cardName.text("None");
    $cardDesc.text(
      `Sadly, there is no official reason to celebrate in ${countryName} on the date in question :(\n We hope you still find many personal reasons to party!`
    );
  }
  display = true;
}
